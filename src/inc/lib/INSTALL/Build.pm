...
sub new {
    ...
    $self->install_base_relpaths('archbin'  => 'arch');
...
sub ACTION_build {
    ...
    #compile with mcc
    if(my $MCC = $self->config_data('MCC')){
	mkdir $self->base_dir().'/blib/archbin';
	mkdir $self->base_dir().'/blib/archbin/other';
	chdir($self->base_dir().'/blib/archbin/other');
	system($MCC, '-mv', $self->base_dir().'/wavecnv_segmenter.m');
	chdir($self->base_dir());
    }
